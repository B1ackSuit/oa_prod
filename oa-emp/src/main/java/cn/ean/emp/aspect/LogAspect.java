...
import cn.ean.emp.util.web.IPUtils;
import cn.ean.emp.util.web.ServletUtils;
...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult)
    {
        try
        {
            ...
            // *========数据库日志=========*//
            SysLogPO operLog = new SysLogPO();
            int ordinal = BusinessStatusEnum.SUCCESS.ordinal();
            operLog.setType(String.valueOf(ordinal));
            // 请求的地址
            String ip = IPUtils.getIpFromRequest(ServletUtils.getRequest());
            operLog.setIpAddress(ip);
            operLog.setUrl(ServletUtils.getRequest().getRequestURI());
            if (loginUser != null)
            {
                operLog.setOperUser(loginUser.getUsername());
            }

            if (e != null)
            {
                operLog.setType(String.valueOf(BusinessStatusEnum.FAIL.ordinal()));
//                operLog.setErrorMsg(StringUtils.substring(e.getMessage(), 0, 2000));
            ...
            System.out.println("LogAspect-handleLog operLog: " + operLog.toString());
     ...
     * @throws Exception
     */
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, SysLogPO operLog, Object jsonResult) throws Exception
    {
        // 设置action动作
        operLog.setType(String.valueOf(log.businessType().ordinal()));
        // 设置标题
        operLog.setDescr(log.title());
